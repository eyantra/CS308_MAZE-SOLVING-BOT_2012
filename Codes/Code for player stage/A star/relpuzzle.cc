...
#define rot90    p2dProxy.SetSpeed(0, dtor(30)); sleep(3);p2dProxy.SetSpeed(0, dtor(0));botDirection=(botDirection+1)%4;
...
void movef(){
robot.Read();
float  x=p2dProxy.GetXPos();
float  y=p2dProxy.GetYPos();
if(botDirection==0)y++;
if(botDirection==1)x++;
if(botDirection==2)y--;
if(botDirection==3)x--;
while(!(abs(x-p2dProxy.GetXPos())<0.01 && abs(y-p2dProxy.GetYPos())<<0.01)){
    p2dProxy.SetSpeed(1, dtor(0));
    usleep(100000);
    p2dProxy.SetSpeed(0, dtor(0));sleep(0);
robot.Read();
}      
}
